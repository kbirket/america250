...
export default function TriviaTab({ member, onToast }) {
  ...
  const [selected, setSelected] = useState({}) // questionId -> { answer, correct, correctAnswer }
  ...
  async function handleAnswer(questionId, optionKey) {
    ...
    // Refresh to unlock next question
  ...
  return (
    <>
      ...
      {isEventOver && (
        <div className="info-box">
          Spirit week has wrapped up. Thanks for celebrating America 250 with us!
        </div>
      )}
      ...
      {!isEventPre && !isEventOver && (
        <>
          <div className="section-label">
            {today} · {data?.questions?.length || 0} questions
          </div>
          ...
          {data?.questions?.map((q, idx) => {
            ...
            const isUnlocked = q.unlocked || (idx === 0) || !!selected[data.questions[idx - 1]?.id]
            ...
            return (
              <div key={q.id} className={`card${!isUnlocked ? '' : ' card-blue'}`} style={{ opacity: isUnlocked ? 1 : 0.5 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  ...
                  {isAnswered && (
                    <span style={{
                      ...
                      color: (sel?.correct ?? q.answered) ? '#2d7a3a' : '#a32d2d',
                    }}>
                ...
                <p style={{ fontSize: 14, color: '#222', lineHeight: 1.5, marginBottom: 12 }}>
                  {q.question}
                </p>
